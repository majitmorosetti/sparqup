...
export default function ResponsivePreview({
  ...
  const frames: Frame[] = useMemo(() => {
    const def: Record<Mode, { w: number; h: number; r: number; label: string }> = {
    ...
    return [
      { mode: "desktop", w: d.w, h: d.h, r: def.desktop.r, label: def.desktop.label },
      { mode: "mobile",  w: m.w, h: m.h, r: def.mobile.r,  label: def.mobile.label  },
    ]
  }, [sizes])
    ...
    async function animateTo(nextIndex: number) {
      ...
      if (prefersReduced || overshoot === 1) {
        await controls.start({
          ...
          transition: { duration: Math.min(0.35, transitionSec), ease: "easeInOut" },
        })
      } else {
  ...
  // ---- NAV items (desktop) ----
  const navItems = ["Accueil", "À propos", "Services", "Tarifs", "Blog", "Contact"]
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Mini site */}
          <motion.div className="grid" layout style={{ gridTemplateRows: "auto 1fr" }}>
            {/* Header du site */}
            <motion.header
            ...
            >
              ...
              <div className="inline-flex items-center gap-2">
                ...
                {!isMobile && <span className="text-sm font-semibold">Sparqup</span>}
              </div>
              ...
              <motion.div layout className="flex items-center gap-2">
                {!isMobile && (
                  <motion.span layout className="inline-block rounded-md border px-3 py-2 text-xs">
                    Contact
                  </motion.span>
                )}
                <motion.span layout className="inline-block rounded-md border px-3 py-2 text-xs">
            ...
            <motion.main layout className="grid gap-6 p-4">
              {/* Hero visuel */}
              <motion.div layout className="h-[220px] rounded-xl border bg-foreground/5" />
              {/* Cards : 2 colonnes en desktop, 1 en mobile */}
              <motion.div
                layout
                className={["grid gap-3", isMobile ? "grid-cols-1" : "grid-cols-2"].join(" ")}
              >
                <motion.div layout className="h-28 rounded-xl border bg-foreground/5" />
                <motion.div layout className="h-28 rounded-xl border bg-foreground/5" />
                <motion.div layout className="h-28 rounded-xl border bg-foreground/5" />
                <motion.div layout className="h-28 rounded-xl border bg-foreground/5" />
              </motion.div>
