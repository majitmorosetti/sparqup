...
export default function Section({
  ...
  const base = [
    ...
    variant === "frosted" && "backdrop-blur-xl backdrop-saturate-150 bg-surface-1/70 supports-[backdrop-filter]:bg-surface-1/40",
  ...
  return (
    <section id={id} className={`py-8 md:py-12`}>
      <div className={`${bleed ? "max-w-none px-0" : "mx-auto max-w-6xl px-4 md:px-6"}`}>
        <div className={base}>
          {(title || subtitle || actions) && (
            <header className="mb-5 flex items-start justify-between gap-4">
              <div>
                ...
                {subtitle && (
                  <p className="mt-1 text-sm md:text-base text-muted">{subtitle}</p>
                )}
              </div>
