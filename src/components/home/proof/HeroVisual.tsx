...
import {
  LayoutDashboard, Mail, Database, Cog, Workflow, Zap, Bot, Plug,
} from "lucide-react"
...
export default function HeroVisual({ className = "" }: Props) {
  ...
  return (
    ...
    >
      {/* ===== FLUX (SVG) — Mobile ===== */}
      <svg className="pointer-events-none absolute inset-0 block md:hidden" viewBox="0 0 1000 600" aria-hidden>
        <defs>
        ...
        <motion.path
          ...
          fill="none" stroke="url(#flux)" strokeWidth="2.4" strokeDasharray="12 10" filter="url(#glow)"
          initial={{ strokeDashoffset: 0 }} animate={lineAnim} transition={lineTrans(6)}
          className="text-foreground/45"
        />
        <motion.path
          d="M 760 270 C 640 360, 520 440, 360 490"
          fill="none" stroke="url(#flux)" strokeWidth="2.4" strokeDasharray="10 8" filter="url(#glow)"
          initial={{ strokeDashoffset: 0 }} animate={lineAnim} transition={{ ...lineTrans(7.5, 0.3) }}
          className="text-foreground/35"
        />
      </svg>

      {/* ===== FLUX (SVG) — Desktop ===== */}
      <svg className="pointer-events-none absolute inset-0 hidden md:block" viewBox="0 0 1200 500" aria-hidden>
        <defs>
          <filter id="glow2" x="-50%" y="-50%" width="200%" height="200%">
            <feGaussianBlur stdDeviation="2.2" result="b" />
            <feMerge><feMergeNode in="b" /><feMergeNode in="SourceGraphic" /></feMerge>
          </filter>
          <linearGradient id="flux2" x1="0" y1="0" x2="1" y2="0">
        ...
        <motion.path
          ...
          fill="none" stroke="url(#flux2)" strokeWidth="2.6" strokeDasharray="14 10" filter="url(#glow2)"
          initial={{ strokeDashoffset: 0 }} animate={lineAnim} transition={lineTrans(7.2)}
          className="text-foreground/45"
        />
        <motion.path
          d="M 240 260 C 460 300, 760 360, 980 380"
          fill="none" stroke="url(#flux2)" strokeWidth="2.6" strokeDasharray="12 9" filter="url(#glow2)"
          initial={{ strokeDashoffset: 0 }} animate={lineAnim} transition={{ ...lineTrans(6.2, 0.25) }}
          className="text-foreground/35"
        />
        <motion.path
          d="M 980 180 C 900 220, 900 320, 980 360"
          fill="none" stroke="url(#flux2)" strokeWidth="2.4" strokeDasharray="10 8" filter="url(#glow2)"
          initial={{ strokeDashoffset: 0 }} animate={lineAnim} transition={{ ...lineTrans(5.6, 0.6) }}
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-12 md:gap-6">
        ...
        >
          <BlockHeader icon={<LayoutDashboard className="h-5 w-5" />} title="Site / Dashboard" />
        ...
        >
          <BlockHeader icon={<Cog className="h-5 w-5" />} title="Digitalisation" />
          <ul className="mt-3 space-y-2 text-sm">
            <LI icon={<Mail className="h-4 w-4 opacity-80" />}>Mailer &amp; DNS</LI>
            <LI icon={<Database className="h-4 w-4 opacity-80" />}>CRM / Contacts</LI>
            <LI icon={<Plug className="h-4 w-4 opacity-80" />}>Outils métier</LI>
          </ul>
        </motion.div>
        ...
        >
          <BlockHeader icon={<Workflow className="h-5 w-5" />} title="Automatisation" />
...
function Chip({ icon, emoji, label }: { icon?: ReactNode; emoji?: string; label: string }) {
  return (
