...
import { motion, useAnimationControls, useMotionValue, useReducedMotion } from "motion/react"
...
export default function ResponsiveDevice({
  ...
  const variants = useMemo(
    () => ({
      desktop: { width: desktop.w, height: desktop.h, borderRadius: desktop.r, transition: spring },
      mobile:  { width: iphone.w,  height: iphone.h,  borderRadius: iphone.r,  transition: spring },
    }),
    ...
    >
      <motion.div
        className="
          ...
          transform-gpu
        "
        style={{ contain: "content", willChange: "width, height, border-radius, transform", scale: scaleMv, transformOrigin: "50% 50%" }}
        variants={variants}
        initial={phase}
        ...
        onUpdate={(latest: any) => {
          ...
          const h = typeof latest.height === "number" ? latest.height : undefined
          if (w && h) {
            ...
            const next =
              cur ? ratio > 0.98 /* revenir desktop si < 0.98 */ : ratio > 1.02 /* passer mobile si > 1.02 */
            if (next !== cur) {
  ...
  return (
    ...
    >
      <div className="flex h-full items-center justify-between gap-2 px-3">
        ...
        <motion.div className="mx-2 h-3 flex-1 rounded bg-white/15" transition={barSpring} />
        <motion.div className="h-3 w-6 rounded bg-white/25" transition={barSpring} />
      </div>
    </motion.div>
  )
}
...
function HeroWireframe({
  ...
  const colorBlock = isMobile ? "#d1d5db" /* neutral-300 */ : "#e5e7eb" /* neutral-200 */
  const colorStrong = isMobile ? "#9ca3af" /* neutral-400 */ : "#d4d4d8" /* zinc-300 */
    ...
    >
      ...
      <div className="flex min-w-0 flex-col">
        ...
        <motion.div
          ...
          transition={spring}
          style={{ height: isDesk ? "14%" : "12%", width: isDesk ? "92%" : "90%" }}
        />
        {/* Description */}
        <motion.div
          ...
          transition={spring}
          style={{ height: isDesk ? "8%" : "7.5%", width: isDesk ? "84%" : "92%" }}
        />
        {/* CTAs */}
        <div className="mt-auto flex items-center gap-[3%]">
          <motion.div
            ...
            animate={{ backgroundColor: colorStrong }}
            transition={spring}
            style={{ height: "10%", minHeight: 28, width: isDesk ? "34%" : "60%" }}
          />
          <motion.div
            className="rounded-md"
            animate={{ backgroundColor: colorBlock }}
            transition={spring}
            style={{ height: "10%", minHeight: 28, width: isDesk ? "24%" : "32%" }}
          />
        </div>
      </div>
